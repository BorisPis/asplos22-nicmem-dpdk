...
static struct rte_ether_addr client_ether_addr =
    {{0x1c, 0x34, 0xda, 0x64, 0x6c, 0x94}};
static uint32_t client_ip_addr = RTE_IPV4(10, 1, 130, 30);

/* the server side */
static struct rte_ether_addr server_ether_addr =
    {{0x1c, 0x34, 0xda, 0x64, 0x6d, 0x10}};
static uint32_t server_ip_addr = RTE_IPV4(10, 1, 130, 100);
...
print_port_statistics(void)
{
    ...
    rte_log(RTE_LOG_INFO, RTE_LOGTYPE_PINGPONG, "%d iters in %.2f seconds = %.2f usec/iter\n",
