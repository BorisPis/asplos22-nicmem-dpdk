...
static uint32_t ctrs_sip = 1;
static uint32_t ctrs_udp_sport = 1;
...
print_usage(const char *prgname)
{
	fprintf(stderr, "%s [EAL options] --"
		...
		"                    Default: 1\n"
		"                    Valid only if --mode=eventdev\n"
		"  --ipv4-ctrs: Number of IPv4 source addr match rules.\n"
		"  --udp-sport-ctrs: Number of UDP source port match rules.\n",
		prgname);
}
...
#define CMD_LINE_OPT_CTRS_SIPV4 "ipv4-ctrs"
#define CMD_LINE_OPT_CTRS_UDP_SPORT "udp-sport-ctrs"
enum {
	...
	CMD_LINE_OPT_CTRS_SIPV4_NUM,
	CMD_LINE_OPT_CTRS_UDP_SPORT_NUM,
};
...
static const struct option lgopts[] = {
	...
	{CMD_LINE_OPT_CTRS_SIPV4, 1, 0, CMD_LINE_OPT_CTRS_SIPV4_NUM},
	{CMD_LINE_OPT_CTRS_UDP_SPORT, 1, 0, CMD_LINE_OPT_CTRS_UDP_SPORT_NUM},
	{NULL, 0, 0, 0}
};
...
parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, short_options,
		...
		case CMD_LINE_OPT_CTRS_SIPV4_NUM:
			ctrs_sip = parse_number_or_zero(optarg);
			break;

		case CMD_LINE_OPT_CTRS_UDP_SPORT_NUM:
			ctrs_udp_sport = parse_number_or_zero(optarg);
			break;
		default:
			print_usage(prgname);
			return -1;
...
init_counters(void)
{
	...
	unsigned lcore_id;
	size_t sz = ctrs_sip * (size_t)ctrs_udp_sport * sizeof(uint64_t);
	void *ctrs1,*ctrs2;
	printf("[+] allocating %lu counters ipv4 mask %08x udp sport mask %08x\n",
	       sz / sizeof(uint64_t), ctrs_sip, ctrs_udp_sport);
	ctrs1 = rte_zmalloc("ctrs", sz, sizeof(uint64_t));
	if (!ctrs1)
		rte_exit(EXIT_FAILURE, "failed to allocated pkt counters\n");
	ctrs2 = rte_zmalloc("ctrs", sz, sizeof(uint64_t));
	if (!ctrs2)
		rte_exit(EXIT_FAILURE, "failed to allocated pkt counters\n");

	for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
		qconf = &lcore_conf[lcore_id];
		qconf->pkt_ctrs = ctrs1;
		qconf->byt_ctrs = ctrs2;
		qconf->ctrs_sipv4 = ctrs_sip;
		qconf->ctrs_udp_sport = ctrs_udp_sport;
		qconf->ctrs_sipv4_shift = rte_log2_u32(ctrs_udp_sport);
	}
	return 0;
}
